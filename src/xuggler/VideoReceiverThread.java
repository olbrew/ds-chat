...
public class VideoReceiverThread implements Runnable {
    ...
    public void run() {
        try {
            ...
            System.err.println("client> Failed creating private receiver socket for video streaming.");
        }
        ...
        while (true) {
            try {
                ...
                if (size > 0) {
                    ...
                    System.out.println("Received " + image.getHeight() + "x" + image.getWidth() + ": " + ms);
                    ImageIO.write(image, "jpg", new File("../resources/images/receiver_" + ms + ".jpg"));
                }
                // TODO display received frames
            } catch (IOException e) {
                System.err.println("client> Failed reading image / frame from the socket.");
                e.printStackTrace();
            }
        }
    }
