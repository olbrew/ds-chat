...
public class ChatServer implements Chat, Runnable {
	...
	public String join(String username, String roomName) throws AvroRemoteException {
		...
		if (roomName.equals("Public")) {
			...
			String request = username + " would like to start a private conversation with you.\n"
	 ...
	 * @throws AvroRemoteException
	 */
	private boolean setupConnection(String client1, String client2) throws AvroRemoteException {
		try {
			System.out.println("Setting up connections between " + client1 + " and " + client2);
			...
			if (((clientsServer.get(client1)).connectToClient(client2, client2Address))
				...
				System.out.println("Connection succesfully made between" + client1 + " and " + client2);
				return true;
			} else {
				System.err.println(
						"Something went wrong with setting up connections between " + client1 + " and " + client2);
				return false;
			}
		} catch (IOException e) {
			e.printStackTrace();
