...
public class ChatClient implements ChatClientServer, Runnable {
	 ...
	 */
	@Override
	public Void setupVideoRequest(boolean privateProxy)
			throws AvroRemoteException {
		if (privateProxy) {
			this.privateProxy.setupVideoRequest(false);
			System.out.println(
					"client> A video request has been sent to the other client.");
		} else {
			awaitingVideo = true;
		}
	 ...
	 */
	@Override
	public Void setupVideoStreaming(boolean privateProxy)
			throws AvroRemoteException {
	 ...
	 */
	@Override
	public boolean register(String username, String privateAddress)
			throws AvroRemoteException {
		...
		try {
			privateTransceiver = new SaslSocketTransceiver(
					new InetSocketAddress(InetAddress.getByName(privateIP),
							privatePort));

			privateProxy = (ChatClientServer) SpecificRequestor
					.getClient(ChatClientServer.class, privateTransceiver);
	...
	public Void leave(boolean closeOtherProxy) throws AvroRemoteException {
		if (closeOtherProxy) {
			privateProxy.incomingMessage("client> " + username
	...
	private void connectToServer() {
		try {
			...
			serverProxy = (Chat) SpecificRequestor.getClient(Chat.class,
					serverTransceiver);
			...
			clientTransceiver = new SaslSocketTransceiver(new InetSocketAddress(
					InetAddress.getByName(clientIP), clientPort));
			clientProxy = (ChatClientServer) SpecificRequestor
					.getClient(ChatClientServer.class, clientTransceiver);

			ShellFactory.createConsoleShell("client", "", new ClientUI(this))
					.commandLoop();
			...
			System.err.println(
					"client> Something went wrong when communicating with the server.");
			System.exit(1);
		}
	}
	...
	private void reconnect(int n) {
		try {
			if (n == 6) {
				System.err
						.println("client> Failed to reconnect to server after "
								+ n + " attempts.\n"
				...
				System.err.println(
						"client> Cannot access the server, trying to reconnect in "
								+ n * 5 + " seconds.");
