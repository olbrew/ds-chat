...
public class ClientUI {
	...
	public void getClientList() {
		try {
			...
			System.err.println(
					"server> Failed to receive answer from the server.");
		}
	}
		...
		try {
			if (client.getClientProxy().inPrivateRoom()) {
				...
				if (client.getServerProxy().leave(client.getUsername())) {
					System.out.println(
							"server> You have left the Public chat room.");
				} else {
					System.err.println(
							"server> You couldn't leave the Public chat room, maybe you never joined it.");
				}
			}
		} catch (AvroRemoteException e) {
			System.err.println(
	...
	public void join(
		...
		try {
			if (client.getClientProxy().inPrivateRoom()) {
				...
				String output = client.getServerProxy()
						.join(client.getUsername(), room);
				System.out.println(output);
			}
		} catch (AvroRemoteException e) {
			System.err.println(
	...
	public void sendMessage(
		...
		try {
			if (client.getClientProxy().inPrivateRoom()) {
				String output = client.getUsername() + "> (Private): "
						+ message;
				client.getClientProxy().sendPrivateMessage(output);
				System.out.println(output);
			} else {
				String output = client.getServerProxy()
						.sendMessage(client.getUsername(), message);
				System.out.println(output);
			}
		} catch (AvroRemoteException e) {
			System.err.println(
	...
	public void video() {
		try {
			if (client.getClientProxy().inPrivateRoom()) {
				String request = "client> " + client.getUsername()
						+ " would like to start video streaming.\n"
			...
			System.err.println(
					"client> Something went wrong using client proxy when requesting video streaming.");
		}
	}
		...
		try {
			if (client.getClientProxy().inPrivateRoom()) {
				if (client.getClientProxy().isAwaitingVideo()) {
					...
					System.out.println(
							"client> You've accepted the video streaming request, enjoy!");
				} else {
			...
			System.err.println(
					"client> Something went wrong using client proxy when accepting video streaming.");
		}
	}
	...
	public void accept(
		...
		try {
			if (client.getClientProxy().inPrivateRoom()) {
				...
				if (client.getServerProxy().setupConnection(chatPartner,
						client.getUsername())) {
				System.out.println(
						"server> Connection set up, you can chat privately now.");
			} else {
				System.err.println(
						"server> Something went wrong with setting up the connection.\n"
								+ "server> Are you sure you got a chat request from "
								+ chatPartner + "?\n" + "server> Check if "
								+ chatPartner
								+ " is still online by typing 'gcl'.\n"
